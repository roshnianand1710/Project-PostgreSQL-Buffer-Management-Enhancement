...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	REPLACEMENT_STRAT = getenv("BUFFER_STRAT"); //FIFO: here replacement_strat stores the either FIFO/CLOCK/NULL.
    if (strcmp(REPLACEMENT_STRAT,"CLOCK")!=0&&strcmp((REPLACEMENT_STRAT,"FIFO")!= 0)||REPLACEMENT_STRAT==NULL) //If replacement_strat is set to null, this block executes and deafults it to FIFO implementation.
    {
        elog(LOG, "Defaulting to FIFO since REPLACEMENT_STRAT is NULL!!!");
        REPLACEMENT_STRAT = "FIFO";
    }
	if (strategy != NULL)
	{
	...
	if (strcmp(REPLACEMENT_STRAT,"CLOCK")==0){
	...
	else if (strcmp(REPLACEMENT_STRAT,"FIFO")==0){ //FIFO: if the REPLACEMENT_STRAT variable is set to FIFO
		while (trycounter>0)
		{
    		buf = GetBufferDescriptor(F_HEAD); 
			local_buf_state = LockBufHdr(buf); //Lock the buffer
			if (BUF_STATE_GET_REFCOUNT(local_buf_state)==0){
				*buf_state = local_buf_state;
				F_HEAD = buf->fLink;
				return buf;
			...
			trycounter--;
            UnlockBufHdr(buf, local_buf_state); //Unlock the buffer
            F_HEAD = buf->fLink;
		}
		elog(ERROR, "Oops , no buffers available!!!");
	}
}
...
{
	F_HEAD = 0; //FIFO: initialising F_HEAD with 0.
	bool		found;
